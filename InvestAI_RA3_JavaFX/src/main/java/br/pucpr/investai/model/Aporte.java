...
public class Aporte implements Identificavel, Validavel {
    ...
    public void validar() {
        ...
        if (metaId <= 0) throw new IllegalArgumentException("ID da meta deve ser maior que zero.");
        if (valor == null || valor.signum() <= 0) throw new IllegalArgumentException("Valor do aporte deve ser maior que zero.");
        if (dataAporte == null) throw new IllegalArgumentException("Data do aporte é obrigatória.");
    }
}
