...
public class OrcamentoCategoria implements Identificavel, Validavel {
    ...
    public void validar() {
        ...
        if (categoriaId <= 0) {
            throw new IllegalArgumentException("ID da categoria deve ser maior que zero.");
        }

        if (limiteMensal == null || limiteMensal.signum() <= 0) {
            throw new IllegalArgumentException("Limite mensal deve ser maior que zero.");
        }
